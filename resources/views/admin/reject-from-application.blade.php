...
<p><a id="back-button" href="{{ url('/admin/company/reject-from-application'.'/'.$jobItemIds.'/'.str_slug($title)) }}"><i class="fa fa-arrow-left"></i> Back</a></p>
@endif
        ...
        <div class="col-md-10">
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              ...
              <li class="pull-right"><a class="text-red" href="{{ url('/admin/company/reject-from-application'.'/'.$jobItemIds.'/'.str_slug($title)) }}"><i class="fa fa-ban" aria-hidden="true"></i> Reject Application ({{ $job_applicants_reject_number }})</a></li>
              <li>
...
function Reject(i){
  ...
  $.post('application_restoring_reject_list',{'job_post_id':jobId,'user_id':i,'_token':$('input[name=_token]').val()},
    function(response){
    $('#reject-applicants'+i).html("");
    ...
    $('#shortlist').load(location.href+' #shortlist');
      if (response.data=='success') {
        $('#message').html('<div class="callout callout-success text-center"><h2>Wow, Great!</h2><h3>You are restored this applicants!</h3></div>');
          $("#message").show().fadeOut(3000).queue(function(n) {
          $(this).hide(); n();
          }); 
      }
