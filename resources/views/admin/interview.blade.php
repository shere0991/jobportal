...
<p><a id="back-button" href="{{ url('/admin/company/interview'.'/'.$jobItemIds.'/'.str_slug($title)) }}"><i class="fa fa-arrow-left"></i> Back</a></p>
@endif
        ...
        <div class="col-md-10">
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              <li class="active"><a href="#activity" data-toggle="tab" class="text-green">Applicants</a></li>
               <li><a class="text-green" href="{{action('Dashboard\Main@downloadExcelFromInterview',$jobItemIds)}}">Download <i class="fa fa-file-excel-o text-green"></i></a></li>
                <li class="pull-right"><a class="text-red" href="{{ url('/admin/company/reject-from-interview'.'/'.$jobItemIds.'/'.str_slug($title)) }}"><i class="fa fa-ban" aria-hidden="true"></i> Reject Application ({{ $job_applicants_reject_number }})</a></li>
                <li>
                    ...
                    <div class="col-sm-12">
                      <div class="row">
                        ...
                        <div class="col-sm-2">
                          ...
                          <span id="reject-applicants{{ $applicants->user_id }}" onclick="Reject('{{ $applicants->user_id }}')" class="btn-box-tool text-danger" title="Reject"><i class="fa fa-times text-red"></i></span><br>
                          <span class="text-aqua">Final Listing</span>
                          {{-- <span>Reject</span> --}}
                          </div>
                        </div>
                        <!-- /.col -->
            ...
            <!-- /.nav-tabs-custom -->
            <div class="pull-right">{{ $job_applicants->links() }}</div>
          </div>
          <!--col-md-10-->
        </div>
        <!-- /.row -->
...
function InterviewList(i){
  ...
  var jobId=$('#jobId').val();
  var status=1;
  $.post('FinalListing',{'applicationIds':applicationIds,'user_id':i,'job_post_id':jobId,'ExpectedSalary':ExpectedSalary,'status':status,'_token':$('input[name=_token]').val()},function(response){
  $('#shortlist'+i).html("");
...
function Reject(i){
  ...
  var jobId=$('#jobId').val();
  $.post('rejectFromInterview',{'job_post_id':jobId,'user_id':i,'_token':$('input[name=_token]').val()},
    function(response){
      $('#reject-applicants'+i).html("");
